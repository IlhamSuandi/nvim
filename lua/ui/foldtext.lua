local function parse_line(linenr)
  ...
  for id, node, metadata in query:iter_captures(tree:root(), 0, linenr - 1, linenr) do
    ...
    if start_row == linenr - 1 and (end_row == linenr - 1 or end_row == linenr) then
...
function HighlightedFoldtext()
  ...
  if tools.nonprog_modes[vim.bo.filetype] then
    return result
  end
...
local function set_fold_hl()
  ...
  vim.api.nvim_set_hl(0, "FoldedText", { bg = nf.bg, fg = directory.fg, italic = true })
end
