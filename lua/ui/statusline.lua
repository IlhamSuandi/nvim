...
local ui_icons = {
  ...
  ["file"] = { "BufferlineNumberSelected", icons["file"] },
  ["recording"] = { "DiagnosticError", icons["recording"] },
...
local function get_path_info(root, fname, icon_tbl)
  ...
  local dir_path = win_width >= dir_threshold_width + #repo_info + #file_icon_name and dir_path or ""
...
-- Recording WIDGET
--- Create a string containing info for recording status
--- @return string: recording status
local function get_recording_status(icon_tbl)
  -- Add recording status
  local recording = vim.fn.reg_recording()
  return recording ~= ""
      and table.concat({
        icon_tbl["file"],
        "%#ErrorMsg#", -- Highlight group
        "Recording @",
        recording,
        "%*", -- Reset highlight
      })
...
local function get_scroll_percentage()
  -- Get the current line and total lines in the buffer
  local cur_line = vim.api.nvim_win_get_cursor(0)[1]
  local total_lines = vim.api.nvim_buf_line_count(0)

  -- Calculate the scroll percentage (0% to 100%)
  local percentage = math.ceil((cur_line / total_lines) * 100)

  -- Return the percentage as a string (e.g., "10%")
  local value = string.format("%d%%", percentage)
  return table.concat({
    value,
    "%",
  })
...
vim.o.statusline = "%#StatusLine#%{v:lua.get_scroll_percentage()}%*"

-- Recording WIDGET
--- Create a string containing info for recording status
--- @return string: recording status
local function get_vim_mode()
  -- Add current Vim mode
  local mode_map = {
    n = "NORMAL",
    no = "OPERATOR",
    v = "VISUAL",
  ...
  local mode_label = mode_map[current_mode] or current_mode

  return table.concat({
    " ",
    "%#StatusLineMode#", -- Highlight group for mode (customize this group in your config)
    mode_label,
    "   ",
    "%*", -- Reset highlight
  })
...
local function get_scrollbar()
  ...
  vim.api.nvim_set_hl(0, "ScrollBar", { fg = "#e1e1e1", bg = "#51576e", bold = true })
  return tools.hl_str("ScrollBar", sbar)
end
...
M.render = function()
  ...
  stl_parts["recording"] = get_recording_status(hl_ui_icons)
